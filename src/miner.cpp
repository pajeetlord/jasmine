...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        int64_t lastAccumulated = nHeight - (nHeight % 10) - 10;
        // fix this from going below 0
        if (lastAccumulated < 0) lastAccumulated = 0;

        //Calculate the accumulator checkpoint only if the previous cached checkpoint need to be updated
        uint256 nCheckpoint;
        uint256 hashBlockLastAccumulated = chainActive[lastAccumulated]->GetBlockHash();
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("JasmineMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        ...
        return error("JasmineMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("JasmineMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("jasmine-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running JasmineMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
