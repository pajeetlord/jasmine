...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeJasmineAmount"))
        settings.setValue("nAnonymizeJasmineAmount", 1000);

    nAnonymizeJasmineAmount = settings.value("nAnonymizeJasmineAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeJasmineAmount"))
        SoftSetArg("-anonymizejasmineamount", settings.value("nAnonymizeJasmineAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in jasmine.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeJasmineAmount:
            return QVariant(nAnonymizeJasmineAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeJasmineAmount:
            nAnonymizeJasmineAmount = value.toInt();
            settings.setValue("nAnonymizeJasmineAmount", nAnonymizeJasmineAmount);
            emit anonymizeJasmineAmountChanged(nAnonymizeJasmineAmount);
            break;
        case CoinControlFeatures:
