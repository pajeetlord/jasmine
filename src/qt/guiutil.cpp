...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Jasmine address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // return if URI is not valid or is no Jasmine: URI
...
{
    // Convert jasmine:// to jasmine:
    //
    //    Cannot handle this later, because jasmine:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open jasmine.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Jasmine.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Jasmine.lnk
...
{
    return GetAutostartDir() / "jasmine.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a jasmine.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Jasmine\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the jasmine app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add jasmine app to startup item list
